...
var type = null;
var _area = false;
this.centroid = new scintilla.Vector2();
this.area = 0;
...
this._recalc = function() {
  ...
  for (i = 0; i < len; i++) {
    ...
    var e = new scintilla.Vector2().copy(p2).sub(p1);
    //e.copy(points[(i + 1) % len]).sub(points[i]);

    var n = new scintilla.Vector2().copy(e).perp().normalize();
...
scintilla.Polygon.makeTriangle = function(width,height) {
 ...
 return new scintilla.Polygon([
   new scintilla.Vector2(-width/2,height/2),
   new scintilla.Vector2(0,-height/2),
   new scintilla.Vector2(width/2,height/2),
 ]);
